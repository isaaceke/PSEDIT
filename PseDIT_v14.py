...
def _execute_run_visible(step, log):
    preview = step.cmd.replace("\n", " ").replace('"', "'")[:70]
    token = uuid.uuid4().hex[:8]
    tmpdir = Path(tempfile.gettempdir())
    batpath = tmpdir / f"psedit_visible_{token}.bat"
    logfile = tmpdir / f"psedit_visible_{token}.log"
    bat = (
        "@echo off\r\n"
        f"title PSEdit \u2014 {preview}\r\n"
        f"({step.cmd}) > \"{logfile}\" 2>&1\r\n"
        "set RC=%ERRORLEVEL%\r\n"
        f"type \"{logfile}\"\r\n"
        "echo.\r\n"
        "echo \u2500\u2500 PSEdit exit: %RC% \u2500\u2500\r\n"
    ...
    batpath.write_bytes(bat.encode("utf-8"))
    started = time.time()
    creation = getattr(subprocess, "CREATE_NEW_CONSOLE", 0) if sys.platform.startswith("win") else 0
    log.append(("info", f"   \u2197 launching visible console: {batpath.name}"))
    try:
        proc = subprocess.Popen(["cmd", "/c", str(batpath)], creationflags=creation)
        proc.wait()
        rc = proc.returncode
    except Exception as e:
        log.append(("err", f"   \u2717 visible launch: {e}"))
        rc = -1
    ...
    try:
        if logfile.exists():
            ...
            output_lines = [ln.rstrip("\r") for ln in raw.split("\n") if ln.strip()]
        else:
            output_lines = ["(no output captured \u2014 log file missing)"]
    except Exception as e:
        output_lines = [f"(failed to read log: {e})"]

    try:
        batpath.unlink(missing_ok=True)
    ...
    finished = time.time()
    log.append(("ok" if rc == 0 else "warn",
                f"   {'\u2713' if rc == 0 else '\u26a0'} exit {rc} (visible console)"))
    result = {
        "id": uuid.uuid4().hex,
        "job_id": getattr(step, "job_id", "") or ("job_" + uuid.uuid4().hex),
        "request_id": getattr(step, "request_id", ""),
...
class NativeDashboard(QMainWindow):
    ...
    def _submit(self, msg):
        ...
        if errs:
            ...
            return
        total = len(script.blocks) + len(script.copy_blocks) + len(script.find_blocks) + len(script.run_blocks) + len(script.tree_blocks)
        if total == 0:
            self._reply({"v": 2, "type": "result", "request_id": request_id, "session_id": session_id,
